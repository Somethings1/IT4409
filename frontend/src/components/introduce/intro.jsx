    ...
    if (isSignup) {
      if (formData.password !== formData.confirmPassword) {
        ...
        fetch("http://localhost:8080/api/v1/auth/register", {
          ...
          body: JSON.stringify({
            username: formData.email,   // hoặc tên đăng nhập riêng nếu có
            password: formData.password,
            name: formData.username,    // Tên người dùng
            confirm: confirm,
            code: formData.code,
          }),
        })
      ...
      fetch("http://localhost:8080/api/v1/auth/login", {
        ...
        body: JSON.stringify({
          username: formData.email, // API mới dùng 'username' thay vì 'email'
          password: formData.password,
        }),
      })
      .then((response) => {
...
// Chuyển đổi dữ liệu theo cấu trúc bạn cần lưu vào cookies
        const userCookieData = {
          _id: userData.id.toString(),  // _id
          name: userData.name,
          email: userData.email,
          password: userData.password ,  // Cần mã hóa password trước khi lưu
          GoogleID: null,
          role: userData.role.name,  // Chỉ lưu tên role (ví dụ: "ADMIN")
          role_id: userData.role.id.toString(),
          createdAt: userData.role.createdAt,  // Thời gian tạo role
          updatedAt: userData.role.updatedAt || userData.role.createdAt,  // Nếu không có updatedAt, sử dụng createdAt
          id_owner: userData.id.toString()  // Giả sử id_owner là id của user
        };
        localStorage.setItem("token", data.data.access_token);
        // Cookies.set("user", JSON.stringify(data.data.user), { expires: 7, sameSite: 'Strict' });
  ...
  const responseMessage = (response) => {
    ...
    startLoading();
    fetch("http://localhost:8080/login/login_google", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
setIsforgot(true);
  ...
  const sentagain = ()=>{
    ...
    console.log(body);
    startLoading();
    fetch("http://localhost:8080/login/sign_up", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
